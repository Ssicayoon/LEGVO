...
#pragma scop
 for (ii = _PB_N-1; ii >= 0; ii--) {
  for (j=ii+1; j<_PB_N; j++) {

   if (j-1>=0)
      table[ii][j] = max_score(table[ii][j], table[ii][j-1]);
   if (ii+1<_PB_N)
      table[ii][j] = max_score(table[ii][j], table[ii+1][j]);

   if (j-1>=0 && ii+1<_PB_N) {
     /* don't allow adjacent elements to bond */
     if (ii<j-1)
        table[ii][j] = max_score(table[ii][j], table[ii+1][j-1]+match(seq[ii], seq[j]));
     else
        table[ii][j] = max_score(table[ii][j], table[ii+1][j-1]);
   }

   for (k=ii+1; k<j; k++) {
      table[ii][j] = max_score(table[ii][j], table[ii][k] + table[k+1][j]);
   }
  }
 }
#pragma endscop
