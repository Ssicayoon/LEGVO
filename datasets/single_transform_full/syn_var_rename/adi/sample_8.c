...
#pragma scop
 ...
 for (t=1; t<=_PB_TSTEPS; t++) {
    ...
    for (ii=1; ii<_PB_N-1; ii++) {
      u[ii][0] = SCALAR_VAL(1.0);
      p[ii][0] = SCALAR_VAL(0.0);
      q[ii][0] = u[ii][0];
      for (j=1; j<_PB_N-1; j++) {
        p[ii][j] = -f / (d*p[ii][j-1]+e);
        q[ii][j] = (-a*v[ii-1][j]+(SCALAR_VAL(1.0)+SCALAR_VAL(2.0)*a)*v[ii][j] - c*v[ii+1][j]-d*q[ii][j-1])/(d*p[ii][j-1]+e);
      }
      u[ii][_PB_N-1] = SCALAR_VAL(1.0);
      for (j=_PB_N-2; j>=1; j--) {
        u[ii][j] = p[ii][j] * u[ii][j+1] + q[ii][j];
      }
    }
  }
#pragma endscop
