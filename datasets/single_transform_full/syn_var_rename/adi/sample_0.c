...
#pragma scop
 ...
 for (t=1; t<=_PB_TSTEPS; t++) {
    ...
    for (ii=1; ii<_PB_N-1; ii++) {
      v[0][ii] = SCALAR_VAL(1.0);
      p[ii][0] = SCALAR_VAL(0.0);
      q[ii][0] = v[0][ii];
      for (j=1; j<_PB_N-1; j++) {
        p[ii][j] = -c / (a*p[ii][j-1]+b);
        q[ii][j] = (-d*u[j][ii-1]+(SCALAR_VAL(1.0)+SCALAR_VAL(2.0)*d)*u[j][ii] - f*u[j][ii+1]-a*q[ii][j-1])/(a*p[ii][j-1]+b);
      }

      v[_PB_N-1][ii] = SCALAR_VAL(1.0);
      for (j=_PB_N-2; j>=1; j--) {
        v[j][ii] = p[ii][j] * v[j+1][ii] + q[ii][j];
      }
    }
    //Row Sweep
    for (i=1; i<_PB_N-1; i++) {
