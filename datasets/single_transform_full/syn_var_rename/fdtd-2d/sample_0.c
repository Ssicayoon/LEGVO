...
#pragma scop
  ...
  for(t = 0; t < _PB_TMAX; t++)
	...
	for (j = 0; j < _PB_NY; j++)
	  ey[ii][j] = ey[ii][j] - SCALAR_VAL(0.5)*(hz[ii][j]-hz[ii-1][j]);
      for (ii = 0; ii < _PB_NX; ii++)
	for (j = 1; j < _PB_NY; j++)
	  ex[ii][j] = ex[ii][j] - SCALAR_VAL(0.5)*(hz[ii][j]-hz[ii][j-1]);
      for (ii = 0; ii < _PB_NX - 1; ii++)
	for (j = 0; j < _PB_NY - 1; j++)
	  hz[ii][j] = hz[ii][j] - SCALAR_VAL(0.7)*  (ex[ii][j+1] - ex[ii][j] +
				       ey[ii+1][j] - ey[ii][j]);
    }
