...
#pragma scop
    for (t = 1; t <= TSTEPS; t++) {
        ...
        for (i = 1; i < _PB_N-1; i++) {
           for (j = 1; j < _PB_N-1; j++) {
               /* Peeled first iteration: k = 1 */
               if (1 < _PB_N-1) {
                   A[i][j][1] =   SCALAR_VAL(0.125) * (B[i+1][j][1] - SCALAR_VAL(2.0) * B[i][j][1] + B[i-1][j][1])
                                + SCALAR_VAL(0.125) * (B[i][j+1][1] - SCALAR_VAL(2.0) * B[i][j][1] + B[i][j-1][1])
                                + SCALAR_VAL(0.125) * (B[i][j][2] - SCALAR_VAL(2.0) * B[i][j][1] + B[i][j][0])
                                + B[i][j][1];
               }
               for (k = 2; k < _PB_N-1; k++) {
