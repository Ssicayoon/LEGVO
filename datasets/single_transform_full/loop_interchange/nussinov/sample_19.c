...
static
void init_array (int n, base POLYBENCH_1D(seq,N,n), DATA_TYPE POLYBENCH_2D(table,N,N,n,n))
{
  int i, j;
  for (i=0; i <n; i++) { seq[i] = (base)((i+1)%4); }
  for (i=0; i <n; i++)
     for (j=0; j <n; j++)
       table[i][j] = 0;
}
...
static
void kernel_nussinov(int n, base POLYBENCH_1D(seq,N,n), DATA_TYPE POLYBENCH_2D(table,N,N,n,n))
{
  int i, j, k;
