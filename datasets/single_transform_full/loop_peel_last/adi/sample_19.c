...
#pragma scop
 ...
 for (t=1; t<=_PB_TSTEPS; t++) {
    ...
    for (i=1; i<_PB_N-1; i++) {
      ...
      /* Peeled last iteration: j = _PB_N-2 */
      p[i][_PB_N-2] = -f / (d*p[i][_PB_N-3]+e);
      q[i][_PB_N-2] = (-a*v[i-1][_PB_N-2]+(SCALAR_VAL(1.0)+SCALAR_VAL(2.0)*a)*v[i][_PB_N-2] - c*v[i+1][_PB_N-2]-d*q[i][_PB_N-3])/(d*p[i][_PB_N-3]+e);
