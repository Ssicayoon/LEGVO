...
#pragma scop
  ...
  for(t = 0; t < _PB_TMAX; t++)
	...
	for (j = 1; j < _PB_NY; j++)
      ...
      for (i = 0; i < _PB_NX - 2; i++)
	for (j = 0; j < _PB_NY - 1; j++)
	  hz[i][j] = hz[i][j] - SCALAR_VAL(0.7)*  (ex[i][j+1] - ex[i][j] +
				       ey[i+1][j] - ey[i][j]);
      /* Peeled last iteration: i = _PB_NX - 2 */
      for (j = 0; j < _PB_NY - 1; j++)
	hz[_PB_NX - 2][j] = hz[_PB_NX - 2][j] - SCALAR_VAL(0.7)*(ex[_PB_NX - 2][j+1] - ex[_PB_NX - 2][j] +
				       ey[_PB_NX - 1][j] - ey[_PB_NX - 2][j]);
    }
