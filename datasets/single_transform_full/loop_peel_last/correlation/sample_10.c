  ...
  for (i = 0; i < _PB_M-1; i++)
    {
      ...
      /* Peeled last iteration of j loop: j = _PB_M-1 */
      if (i+1 <= _PB_M-1)
        {
          corr[i][_PB_M-1] = SCALAR_VAL(0.0);
          for (k = 0; k < _PB_N; k++)
            corr[i][_PB_M-1] += (data[k][i] * data[k][_PB_M-1]);
          corr[_PB_M-1][i] = corr[i][_PB_M-1];
        }
    }
