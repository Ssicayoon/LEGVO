...
static
void init_array (int n, DATA_TYPE POLYBENCH_2D(u,N,N,n,n))
{
  int i, j;
  for (i = 0; i < n; i++)
    for (j = 0; j < n; j++)
...
static
void print_array(int n, DATA_TYPE POLYBENCH_2D(u,N,N,n,n))
{
  int i, j;
...
#pragma scop
 ...
 for (t=1; t<=_PB_TSTEPS; t++) {
    ...
    //Row Sweep - merged two adjacent i-loops into one
    for (i=1; i<_PB_N-1; i++) {
